...
public abstract class GElement implements ListModel<Object>, Iterable<GCode> {
    ...
    /**
     * Used by the GCode editor (JList). Can return size()+1 to allow new gcode
     * line to be appened at the end.
     *
    ...
    /**
     * Warns all listeners that the content of this GElement has changed (for
     * JListViewer)
     */
    ...
    public double getAngleInDegre(Point2D center, Point2D p1, Point2D p2) {
        ...
        if ((Math.abs(a) < 0.0000001) || (Math.abs(b) < 0.0000001)) {
            return 0;
        }
    ...
    /**
     * Read a double value in a string that can be a parameter line like "[toto = ]34.5".
     *
     * @param value
     * @return the value or NaN
     */
    public double parseParameterDouble(GCode value) {
     ...
     * @param tolerance the maximal distance between borderd of both objects to
     * contac them
     ...
     *
     * @return A true GCode with G in {1,2,3,8x}, or null if there is no points
     * in this path.
     */
     ...
     */
    abstract public boolean movePoint(GCode point, double dx, double dy);
    ...
    /**
     * @return a G1 path or group composed of G1Paths and GGroups (all clones)
     */
     ...
     * (The <i>GElement.loadFromStream()</i> methode read only properties line
     ...
     *
     * @param stream
     * @param line the line to read from, or can be null then it will use the
     * stream
    ...
    private static GElementFactoryInterface gElementFactory = (String line) -> {
        if (line.startsWith(GGroup.HEADER_STRING)) {
            return GGroup.class;
        }
        ...
        if (line.startsWith(GSphericalPocket.HEADER_STRING)) {
            return GSphericalPocket.class;
        }
        if (line.startsWith(GDrillPoint.HEADER_STRING)) {
            return GDrillPoint.class;
        }
        if (line.startsWith(GSpline.HEADER_STRING)) {
            return GSpline.class;
        }
        if (line.startsWith(GTextOnPath.HEADER_STRING)) {
            return GTextOnPath.class;
        }
        return null;
    };
     ...
     *
     * @param newBuilder
     * @return the old defined (to use in new builder to handle all actual
     * GElement)
     */
    ...
    /**
     * @param line read from project file
     * @return true if the line is the header of a GElement
     */
    ...
    /**
     * Use GElementFactory to make a new GElement based on a
     * <i>GElement.HEADER_STRING</i> string.<br>
